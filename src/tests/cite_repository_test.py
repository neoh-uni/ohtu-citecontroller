...
class TestCiteRepocitory(unittest.TestCase):
    ...
    def test_new_book(self):
        ...
        self.assertEqual([books[0].author, books[0].title, books[0].year, books[0].publisher],
                         ["Test Author", "Title", 2022, "Publisher"])

    def test_new_article(self):
        cite_repository.new_article(Article("Test Author", "Journal", "Title", "2020", "10", "10-12"))
        articles = cite_repository.get_articles()

        self.assertEqual(len(articles), 1)
        self.assertEqual([articles[0].author, articles[0].journal, articles[0].title, articles[0].year, articles[0].volume, articles[0].pages],
                        ["Test Author", "Journal", "Title", 2020, 10, "10-12"])

    def test_new_inproceedings(self):
        cite_repository.new_inproceedings(Inproceedings("Test Author", "Title", "2022", "Booktitle"))
        inproceedings = cite_repository.get_inproceedings()

        self.assertEqual(len(inproceedings), 1)
        self.assertEqual([inproceedings[0].author, inproceedings[0].title, inproceedings[0].year, inproceedings[0].booktitle], 
